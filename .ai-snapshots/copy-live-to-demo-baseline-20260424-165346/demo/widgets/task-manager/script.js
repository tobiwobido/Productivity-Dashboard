...
var DEMO_RESET_REQUESTED = new URLSearchParams(window.location.search).get('resetDemo') === '1';
...
function migrateIfNeeded() {
  if (localStorage.getItem(TM_KEY) && !DEMO_RESET_REQUESTED) return;
  localStorage.setItem(TM_KEY, JSON.stringify(getDemoTaskData()));
}

function demoTask(id, text, priority, dueOffset, done, notes) {
  return {
    id: id,
    text: text,
    done: !!done,
    priority: priority,
    dueDate: typeof dueOffset === 'number' ? demoISO(dueOffset) : null,
    completedAt: done ? Date.now() - Math.floor(Math.random() * 9000000) : null,
    noteItems: notes || [],
    children: []
  };
}

function demoNote(id, text, done) {
  return { id: id, text: text, done: !!done };
}

function getDemoTaskData() {
  return {
    life: [
      demoTask('demo-life-overdue-high', 'Renew car registration', 'high', -3, false, []),
      demoTask('demo-life-today-medium', 'Meal prep lunches for the week', 'medium', 0, false, [
        demoNote('demo-life-note-1', 'Check pantry before shopping', false)
      ]),
      demoTask('demo-life-no-date-low', 'Organize hallway closet', 'low', null, false, []),
      demoTask('demo-life-done-notes', 'Book annual dentist cleaning', 'medium', -6, true, [
        demoNote('demo-life-done-note-1', 'Confirm insurance details', true),
        demoNote('demo-life-done-note-2', 'Add appointment to calendar', true)
      ])
    ],
    work: [
      demoTask('demo-work-today-high', 'Send launch recap to team', 'high', 0, false, [
        demoNote('demo-work-note-1', 'Include open blockers', true),
        demoNote('demo-work-note-2', 'Add next milestone dates', false)
      ]),
      demoTask('demo-work-future-medium', 'Review Q2 forecast workbook', 'medium', 4, false, []),
      demoTask('demo-work-overdue-low', 'Archive vendor invoices', 'low', -1, false, []),
      demoTask('demo-work-complete-high', 'Finalize onboarding checklist', 'high', -2, true, [])
    ],
    pd: [
      demoTask('demo-pd-four-notes', 'Audit listing bundle candidates', 'high', 2, false, [
        demoNote('demo-pd-four-1', 'Export Keepa shortlist', true),
        demoNote('demo-pd-four-2', 'Check pack-size math', false),
        demoNote('demo-pd-four-3', 'Verify gated categories', false),
        demoNote('demo-pd-four-4', 'Save top five products', true)
      ]),
      demoTask('demo-pd-all-notes-complete', 'Update supplier contact sheet', 'medium', null, false, [
        demoNote('demo-pd-all-1', 'Confirm rep email', true),
        demoNote('demo-pd-all-2', 'Add payment terms', true),
        demoNote('demo-pd-all-3', 'Attach latest price list', true)
      ]),
      demoTask('demo-pd-due-today-low', 'Reprice slow-moving demo SKU', 'low', 0, false, []),
      demoTask('demo-pd-completed-medium', 'Save sample COGS products', 'medium', -4, true, [
        demoNote('demo-pd-complete-note', 'Use realistic tax and discount examples', true)
      ])
    ]
  };
}
...
function buildPriDropDOM() {
  ...
  el.innerHTML = '<div class="pri-drop-popup">'
    ...
    + '<div class="pri-drop-item pri-medium" onclick="selectPriority(\'medium\')">Medium</div>'
...
function addNoteItem(tab, taskId, text) {
  text = (text || '').trim();
  if (!text) return;
  var data = getData();
  ...
  container.innerHTML = displayTasks.map(function(task) {
    ...
    var addNoteRow = openNotes[task.id]
      ? '<div class="note-add-row">'
        ...
        +   ' onkeydown="if(event.key===\'Enter\')addNoteItem(\''+tab+'\',\''+task.id+'\',this.value)">'
        +   '<button class="note-add-btn" onclick="addNoteItem(\''+tab+'\',\''+task.id+'\',document.getElementById(\'note-add-'+task.id+'\').value)">+</button>'
        + '</div>'
      : '';
    ...
    return '<div class="task-group" data-id="'+task.id+'">'
      + '<div class="'+ic+'">'
